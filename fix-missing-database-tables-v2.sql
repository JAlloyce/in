-- Fix Missing Database Tables - Version 2
-- This script properly handles existing tables and adds missing columns/tables

-- First, let's check and create the connections table (which doesn't exist)
...
-- Now handle the notifications table - add missing columns if they don't exist
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE public.notifications ADD COLUMN connection_id UUID;
    END IF;
...
-- Create the notifications table if it doesn't exist at all (fallback)
...
-- Now add the foreign key constraint (connection_id column should exist now)
DO $$ 
BEGIN
    IF NOT EXISTS (
...
-- Create RLS Policies for notifications (drop existing ones first to avoid conflicts)
...
-- Create RLS Policies for connections (drop existing ones first to avoid conflicts)
...
-- Create notification triggers (drop existing ones first)
DO $$ 
BEGIN
...
-- Add updated_at triggers for new tables (drop existing ones first)
